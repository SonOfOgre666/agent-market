...
OLD_API_URL=$(grep "^NEXT_PUBLIC_API_URL=" "$ENV_FILE" | cut -d= -f2- || true)
...
if grep -q '^API_URL=' "$ENV_FILE"; then
  sed -i "s|^API_URL=.*|API_URL=${NGROK_API_URL}|" "$ENV_FILE"
fi
...
if [ "$OLD_SC_HOST" != "$CF_RT_HOST" ] || [ "$OLD_API_URL" != "$NGROK_API_URL" ] || [ "$OLD_WEB_URL" != "$CF_WEB_URL" ]; then
  log "Web/realtime/API public URL changed — rebuilding web container..."
  docker compose build web
  docker compose up -d web
  ok "Web container rebuilt"
else
  ok "Public URLs unchanged — no web rebuild needed"
fi
